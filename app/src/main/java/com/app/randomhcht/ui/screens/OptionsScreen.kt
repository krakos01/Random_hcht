...
@Composable
fun CarListItem(carOption: Triple<Int, Int?, String>, cdOption: Boolean) {
    var rememberedOption by remember { mutableStateOf(cdOption) }

    ListItem(
        headlineContent = {
            Column {
                Text(text = stringResource(id = carOption.first))
                if (carOption.second != null) {
                    Text(
                        text = stringResource(id = carOption.second!!),
...
fun DisplayCarsOptions(
    ...
    if (showWarning) MoreRacesThanCarsWarningDialog(onDismissDialog = { showWarning = false })
    ...
    Column {
        Row {
            ...
            IconButton(
                ...
                Icon(imageVector = Icons.Default.Warning, contentDescription = "Cars warning")
            }
        }
...
@Composable
fun MoreRacesThanCarsWarningDialog(
    onDismissDialog: () -> Unit,
    modifier: Modifier = Modifier
) {
    AlertDialog(
        onDismissRequest = onDismissDialog,
        modifier = modifier,
        text = { Text(stringResource(id = R.string.Cars_Warning)) },
        confirmButton = {
            TextButton(
                onClick = onDismissDialog,
            ) {
